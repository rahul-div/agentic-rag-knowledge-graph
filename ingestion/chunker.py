...
class ChunkingConfig:
    """Configuration for chunking (optimized for Gemini)."""

    chunk_size: int = 800  # Optimized for Gemini token limits
    chunk_overlap: int = 150  # Optimized overlap for Gemini
    max_chunk_size: int = 1500  # Reduced max size for Gemini
    ...
    def __post_init__(self):
        ...
        # Log Gemini-optimized settings
        logger.info(
            f"ChunkingConfig initialized for Gemini: chunk_size={self.chunk_size}, overlap={self.chunk_overlap}"
        )
...
class SemanticChunker:
    ...
    async def _split_long_section(self, section: str) -> List[str]:
        ...
        try:
            prompt = f"""
            Split the following text into semantically coherent chunks for Gemini AI processing. Each chunk should:
            1. Be roughly {self.config.chunk_size} characters long (optimized for Gemini)
            2. End at natural semantic boundaries
            3. Maintain context and readability
            4. Not exceed {self.config.max_chunk_size} characters
            5. Be suitable for Gemini's token processing limits
...
def create_chunker(config: ChunkingConfig):
    """
    Create appropriate chunker based on configuration (optimized for Gemini).
    ...
    if config.use_semantic_splitting:
        logger.info("Creating SemanticChunker with Gemini LLM support")
        return SemanticChunker(config)
    else:
        logger.info("Creating SimpleChunker for fast processing")
        return SimpleChunker(config)


# Example usage
async def main():
    """Example usage of the chunker with Gemini optimization."""
    config = ChunkingConfig(
        chunk_size=800,  # Gemini-optimized size
        chunk_overlap=150,  # Gemini-optimized overlap
    ...
    sample_text = """
    # Big Tech AI Initiatives (Gemini-Powered Analysis)
    ...
    - Natural language processing with embedding-001
    ...
    Google's Gemini models compete directly with these offerings.
    """
